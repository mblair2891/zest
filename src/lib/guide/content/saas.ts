  ...
  topic({
    ...
    blocks: [
      ...
      ul(
        ...
        "PIN roles: Server, Bartender, Host (stand), Kitchen, Busser, Cashier. Floor staff can be added from Users with name, PIN, role, and home entity. Password users never use a PIN to open the dashboard.",
      ),
      steps(
        "Open the venue → Users (location owner, manager, or location admin — not a kitchen/server/bartender PIN).",
        "Add Location admin (whole venue) or Entity admin (one selling entity): name, email, temporary password, force change on first login. They sign in at app.summex.app/login — never a PIN pad, never platform CRM.",
        "Floor staff: the list shows name, role, entity, clock, and the 4-digit PIN (shown by default). Reset PIN confirms, then displays the new code. Disable keeps the row; that PIN no longer signs in.",
        "Password logins stay in a separate list. Account passwords are never shown. Isolated demo: Platform Admin may view PINs for support.",
      ),
      tip(
      ...
      related("login", "sections", "platform-admin", "create-org", "platform-tenants", "venue-users"),
    ],
  }),
  topic({
