...
export const ROLE_GUIDE_TOPICS: GuideTopic[] = [
  ...
  topic({
    ...
    chapterId: "roles",
    title: "Host stand",
    summary: "Floor, Checks, Menu, Pay. Waitlist, to-go, and clock under Checks. Not server till close.",
    roles: ["server", "owner_manager"],
    keywords: ["host stand", "seat", "waitlist", "reservation", "to-go", "host PIN", "bar tab"],
    openView: "waitlist",
    blocks: [
      why(
        "The host stand runs the door. After PIN you pick one job. To-go is a menu item — not the only home, and not a bar-tab POS.",
      ),
      ul(
        "Host, supervisor, or manager PIN: Floor, Checks, Menu, Pay. Waitlist, To-go, and Close out are under Checks → Other jobs. The floor shows clocked in HH:MM. No sale when this host stand is bound to a Receipt printer.",
        "To-go uses the same pay and print path as an order-station to-go check.",
        "Bar tab is not on the host menu. No ODS rail, no menu editor.",
        "Kitchen / cook PIN on a host tablet: Close out (End shift) and Done only. No to-go.",
        "Server PIN on a host tablet: Floor / seat + To-go only if “Servers may use the host stand” is on. Otherwise clock + use an order tablet.",
        "You do not 86 another entity’s recipes by default. You do not close a server’s till.",
      ),
      steps(
        "PIN in on a host station. You land on Floor. The bar is Floor, Checks, Menu, Pay.",
        "Seat from the map. The map is a status-colored block and a number. One tap opens the table sheet.",
        "Tap To-go for a named takeout check. Pay and print as on an order tablet.",
      ),
      warn("PIN is not clock-in. A kitchen PIN cannot open to-go from the host stand. A host PIN cannot bump ODS or edit menus."),
      related("floor-pin-login", "role-server", "kiosk-waitlist", "shift-allowables", "host-stand", "device-roles"),
    ],
  }),
  topic({
    id: "role-server",
    ...
    blocks: [
      ...
      steps(
        "PIN in on a full-service order station. The bar is Floor, Checks, Menu, Pay. Menu shows categories first; a tap adds to the current check. Cash and card stay on the pad. Send is always on screen and returns to Floor. To-go, Bar tab, and Close out are under Checks → Other jobs. The floor shows clocked in HH:MM. Counter is still New ticket. Drive-through is still the lane.",
        "Floor is the map and status only — colored blocks, number only, no chairs. One tap reopens a sat table. Pay is the Pay tab: tenders and Print check.",
        "Bar tab opens your assigned section stools — tap a stool. To-go does not use the floor.",
        "Pay when the house allows. Close out is under Checks → Other jobs. Blind count and tip-out recs run first, then the punch.",
      ),
  ...
  topic({
    ...
    blocks: [
      ...
      steps(
        "PIN into an ODS tablet. You see this station’s tickets plus Clock. There is no POS hamburger.",
        "Start when you begin. Bump when ready. Do not take a card from the ODS.",
        "If you PIN an order or host tablet by mistake: Close out and Done only, then use the kitchen display.",
      ),
      warn(
  ...
  topic({
    ...
    blocks: [
      ...
      ul(
        ...
        "PIN ≠ clock ≠ the till count. The pad has Enter and Clock in — not a Login / Clock in / Clock out switch, and no Clock out key. Enter opens the station. Clock in punches and stays on the pad. A wrong PIN shakes and says Invalid PIN. After Enter, if you are off the clock and inside that entity’s allowed clock-in window for today’s published shift, a prompt also offers Clock in for this shift? — Clock in or Not now. Outside the window: no prompt; use Clock in on the pad. Already clocked in: Enter goes to work and the floor shows clocked in HH:MM. Password login never shows it. Clock out is Close out.",
      ...
      steps(
        ...
        "Clock in from Clock in on the PIN pad. Completing clock in does not open order entry. After Enter, you may also accept Clock in for this shift? when you are inside the window. Not now does not punch. Already clocked in, Enter goes to work.",
        "Close out from the in-shift menu. A till (server, bartender) blind-counts and confirms tip-out recs, then punches out. Kitchen and busser: End shift punches out only.",
  ...
  topic({
    ...
    roles: ["owner_manager", "host_operator", "vendor_operator"],
    keywords: ["schedule", "payroll", "shifts", "overtime", "tips", "operator", "adp", "intuit", "csv", "clock"],
    openView: "schedule",
    blocks: [
      why(
        "The food operator’s week is not the bar operator’s week. Each selling entity owns employees, shifts, pay period, clock rules, overtime flags, and labor % on owned lines.",
      ),
      ul(
        ...
        "Grid: week or pay period. Role, entity, optional station or section, start, end, break. Copy last week. Publish week publishes that entity only. Unpublished drafts do not appear on the clock.",
        ...
        "Hours export is entity-scoped CSV/PDF (regular vs OT flags from owner-entered venue rules). If a payroll provider is connected, send automatic / after review / manual. Summex does not calculate net pay, file taxes, or move money.",
  ...
  topic({
    ...
    blocks: [
      ...
      ul(
        ...
        "Labor uses share income is off by default. When on, only the entity that receives the share adds that income to labor sales. The entity that pays stays on its own item sales.",
      ),
      steps(
