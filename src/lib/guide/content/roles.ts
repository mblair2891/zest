...
export const ROLE_GUIDE_TOPICS: GuideTopic[] = [
  ...
  topic({
    ...
    blocks: [
      ...
      ul(
        "Host, supervisor, or manager PIN: short menu — Floor / seat, Waitlist, To-go, Clock in/out, No sale.",
  ...
  topic({
    ...
    blocks: [
      ...
      steps(
        "PIN in on an order station. Full-service menu: My tables, New table, To-go, Bar tab (if the house has a rail), Clock, No sale, Closeout. Counter is New ticket. On an 8\" handheld the item menu is full width; tap Check for the slide-over.",
  ...
  topic({
    ...
    blocks: [
      ...
      ul(
        ...
        "PIN ≠ clock ≠ closeout. The pad logs you into the station. After PIN you see a short menu of jobs this device × this PIN allows — not a dashboard. Clock in / Clock out is on that menu (or the ODS clock bar) — completing clock does not open order entry. After PIN, if you are off the clock and inside that entity’s allowed clock-in window for today’s shift, a prompt offers Clock in for this shift? — Clock in or Not now. Outside the window: no prompt; use Clock in on the station. Already on the clock: no prompt. Password login never shows it.",
