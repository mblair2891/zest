...
export const ROLE_GUIDE_TOPICS: GuideTopic[] = [
  ...
  topic({
    ...
    chapterId: "roles",
    title: "Host stand",
    summary: "Floor + waitlist/seat. New to-go is a persistent action. Not server till close.",
    roles: ["server", "owner_manager"],
    keywords: ["host stand", "seat", "waitlist", "reservation", "to-go", "host PIN", "bar tab"],
    openView: "waitlist",
    blocks: [
      why(
        "The host stand runs the door. To-go is an action on that glass — not the only home, and not a bar-tab POS unless the house turns that on.",
      ),
      ul(
        "Host, supervisor, or manager PIN: home is floor + waitlist / seat.",
        "Persistent action: New to-go order — same pay and print path as an order-station to-go check.",
        "Bar tab is not the host default. Location setting “Host may open bar tabs” turns it on.",
        "Kitchen / cook PIN on a host tablet: clock only. No to-go.",
        "Server PIN on a host tablet: seat + to-go only if “Servers may use the host stand” is on. Otherwise clock + use an order tablet.",
        "You do not 86 another entity’s recipes by default. You do not close a server’s till.",
      ),
      steps(
        "PIN in on a host station. Home is the floor map and Waitlist — not a two-button to-go POS.",
        "Seat from the map. Check in waitlist and reservations.",
        "Tap New to-go order for a named takeout check. Pay and print as on an order tablet.",
      ),
