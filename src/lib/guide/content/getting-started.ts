...
export const GETTING_STARTED_TOPICS: GuideTopic[] = [
  topic({
    ...
    blocks: [
      ...
      p(
        "Summex is the hospitality operating system for a single restaurant or bar, a host venue with multiple operators, or a multi-unit group. The product is Summex. The company is Quantum Reach. Guest cards run through Quantum Payments. Gift cards stay on the first-party Summex ledger. Software billing is separate from card processing. Footer: Summex © Quantum Reach.",
      ),
      p(
      ...
      ul(
        ...
        "Floor PIN signs a person onto this station when they tap Enter. Clock in on that pad only punches. Clock out is Close out — a till counts blind and confirms tip-outs, then punches out. Kitchen and busser Close out is End shift.",
  ...
  topic({
    ...
    blocks: [
      ...
      steps(
        "Back office: Log in at app.summex.app/login with username or email and password. Each password role lands on its own dashboard (tiles for subscribed modules). Platform Admin: CRM / pipeline / tenants / platform settings. Host owner: venue health, all entities, Devices, Publish, combined and per-entity reports. Venue admin (shared venue): the same minus host-merchant chrome. Entity owner: that brand’s sales, labor %, 86, menu, invoices, schedule, payout, payments. Entity manager: those ops plus floor from back office — no billing. Accountant: reports, hours export, gift liability — no Devices, no 86. Never the staff PIN pad. PIN roles stay on the device-role home (order floor/queue, ODS rail, host stand). A kitchen PIN does not turn an order tablet into server UI.",
        "Prime each Android tablet once: install Summex Station, type the one-time Devices code (internet required). Thereafter the device is PIN-only for that venue and role.",
        ...
        "The pad is 1–9, Enter, 0, Clock in. Enter opens the station. Clock in only punches. A wrong PIN shakes and says Invalid PIN. There is no Clock out key. After Enter, if you are off the clock and inside that entity’s allowed early/late window for today’s published shift, the tablet may ask Clock in for this shift? Not now skips the punch. Outside the window there is no prompt; use Clock in on the pad. Already clocked in: Enter goes to work and the floor shows clocked in HH:MM. Password /login never shows this.",
        "Close out is on the in-shift menu. Servers and bartenders blind-count the till and confirm tip-out recs, then the punch. Kitchen and busser Close out is End shift (punch out only).",
  ...
  topic({
    ...
    blocks: [
      ...
      steps(
        ...
        "Upload a whole menu from Menu (PDF, photo, DOCX, or pasted text). That draft belongs to the selling entity you have open. Save / Publish is what stations receive.",
        "If cash discount is on, a single price on Describe with AI is cash. Assist asks cash or card when the basis is open. The menu stores cash; card is computed.",
