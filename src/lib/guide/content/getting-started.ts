...
export const GETTING_STARTED_TOPICS: GuideTopic[] = [
  topic({
    ...
    blocks: [
      ...
      p(
        "The first location is created only through SaaS onboarding. Summex onboards the host. The host then invites operator tenants. There are no public demo houses and no seeded catalogs.",
      ),
      shot(
      ...
      ul(
        ...
        "Revision · 30 Oct 2026 — Receipts by vendor. White paper lockstep. Tip pools: individual, tip-out, FOH / bar / team / dual. Policy only.",
      ),
      steps(
  ...
  topic({
    ...
    summary: "A fresh Summex has no tenants. The first house appears after SaaS onboarding.",
    roles: ["platform_admin", "owner_manager", "host_operator"],
    keywords: ["empty", "first org", "bootstrap", "no demo", "getting started", "onboard"],
    blocks: [
      why(
        "Production Summex does not ship a restaurant. If the tenant list is empty, that is correct until onboarding finishes.",
      ),
      steps(
  ...
  topic({
    ...
    blocks: [
      ...
      steps(
        ...
        "Complete Get pricing → quote → onboarding to create the first house.",
        "Invite the owner. Open POS on this host.",
        "Add staff PINs on that location. There is no skip-password picker and no PIN 0000 seed.",
      ),
      warn(
        "Named seed houses (including any historical “Laundry” catalog) are removed. Factory reset keeps Platform Admin only.",
      ),
      related("empty-start", "onboarding-wizard", "factory-reset", "go-live-ops"),
    ],
  }),
  topic({
    ...
    summary: "Onboard a real multi-operator house through SaaS. No seeded demo catalog.",
    visibility: "platform",
    roles: ["platform_admin", "owner_manager", "host_operator"],
    keywords: ["test venue", "onboarding", "multi-operator", "host", "laundry"],
    openView: "hall",
    blocks: [
      why(
        "Host capture and operator settlement are exercised on a real org you onboard — not a tagged demo row.",
      ),
      p(
        "Prospects go through Get pricing. Use Host Venue / Operator A / Operator B unless they bring their own names.",
      ),
      steps(
        "Get pricing → describe a host + two operators.",
        "Complete host onboarding so the location is a real org (training).",
        "Host invites operators. Each POC sets a password and completes their details.",
        "Open POS. Add menu for each operator, seat, pay once on Quantum Payments (sandbox in training).",
        "Ledger shows host capture and allocations per operator. Settle the period.",
      ),
      warn("There is no Load demo control and no universal PIN. An empty tenant list after Admin login is correct until onboarding."),
      cta("/get-pricing", "Start onboarding", "Intake creates the first real location."),
      related("partner-demo", "type-food-hall", "empty-start", "system-ledger", "host-capture", "tenant-invites"),
    ],
  }),
  topic({
    id: "setup-by-voice",
