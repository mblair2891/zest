...
export const GETTING_STARTED_TOPICS: GuideTopic[] = [
  ...
  topic({
    ...
    blocks: [
      ...
      steps(
        "Back office: Log in at app.summex.app/login with username or email and password. Admin lands on the platform dashboard (CRM / tenants). A location admin (venue owner) lands on that venue’s settings — Overview, Devices, Menus, Publish. An entity admin lands on that selling entity’s dashboard — menu, recipes, costs, schedule, reports, 86, staff PINs — not the other brand’s menu, payout, or labor. Never the staff PIN pad, never /venue/food_hall, never the sales home, CRM, or other tenants.",
