...
export const ORDER_TOPICS: GuideTopic[] = [
  topic({
    ...
    blocks: [
      ...
      steps(
        "Open Menu. Create categories (e.g. Plates, Sandwich, Sides, Dessert, Cocktails). Each group has an order destination and an optional order printer.",
  ...
  topic({
    ...
    summary: "Menu groups map to a destination and printer. One Send, one slip per line.",
    ...
    blocks: [
      ...
      ul(
        "Each menu group (Plates, Sandwich, Sides, Dessert, Cocktails, Beer, …) has an order destination: Kitchen, Bar, Expo, Window, Prep, Salad, Pizza, Dessert, Other, or a house name — plus an optional specific order printer.",
        "Default for one kitchen: every food group → Kitchen → that one order printer. Drinks → Bar if a bar printer exists, else the kitchen printer.",
        "Send collapses lines that share destination + printer into one slip: header, then qty + name + modifiers, one cut. Split paper only when destination or printer differs (salad vs grill vs bar).",
      ...
      steps(
        ...
        "Send fires LAN 9100 from this station (or a queued docked host/ODS). Food groups mapped to destination Kitchen print on that entity’s Kitchen order printer as one ticket. Drinks print on the Bar printer only.",
