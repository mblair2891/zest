  ...
  topic({
    ...
    blocks: [
      ...
      steps(
        ...
        "Receipt printer: assign to floor sections (and optionally Bar tabs / no section). Venue default is to-go / will-call. Cash drawer kick yes/no. Stations that may kick drawer (terminals only). A station may list a fallback receipt printer if the section has none — that is not routing. ODS never.",
      ...
      ul(
        ...
        "Star SP742 at 192.168.0.105:9100: Order printer, destination Kitchen (or Bar / Expo / Window / Prep / Salad / Pizza / Dessert), make/model Star SP700 / SP742. That preset is text-only 9-pin impact (Star Line 7x9, CP437, single pass, no GS graphics, no TM-T20 payload). Kitchen ticket: venue, destination, server, time, entity, qty + item. Header may be double-height only. Staff Send from a paired order tablet writes TCP 9100. Guest Print check is the receipt printer (example 192.168.0.112), not this Star. QR / kiosk / online fire kitchen bytes via the venue queue. Entity filter is Hearth vs Copper; Destination is the line on that side.",
