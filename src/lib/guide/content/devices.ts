...
export const DEVICE_TOPICS: GuideTopic[] = [
  ...
  topic({
    ...
    blocks: [
      ...
      warn(
        "There is no Kitchen printer, Bar printer, or Label printer type. Those are destinations on an Order printer. Star SP700/SP742 tickets are impact text (7x9, no GS bitmaps, no leading !, not doubled thermal). Kitchen tickets never use window.print. Guest checks print on the Receipt printer. QR, kiosk, and online do not print from the guest browser — they queue. Dock a host or ODS (or run the print agent on the house hub) so the queue drains. Do not put order printers on the printer’s own Wi‑Fi.",
      ),
