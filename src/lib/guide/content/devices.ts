  ...
  topic({
    ...
    blocks: [
      ...
      ul(
        ...
        "Order tablet menu — My tables (full-service floor only; map + status, no side drawers). New table. To-go. Bar tab if the venue has a rail and the PIN allows it. Clock in/out. No sale (open drawer on the bound receipt printer; not Pay). Closeout (server / manager, end of shift). Kitchen PIN on an order tablet: clock sheet — Clock and Done only.",
        "Host tablet menu — Floor / seat, Waitlist, To-go, Clock in/out, No sale. No ODS rail, no menu editor, no bar tab on this menu.",
  ...
  topic({
    ...
    blocks: [
      ...
      steps(
        ...
        "Receipt printer: cash drawer kick yes/no; which order/host stations use it (empty = every order and host station); print pay QR when the venue QR setting is on. ODS does not need a receipt printer. The order pad shows “Add a receipt printer” only when THIS station has no bound receipt printer — not because the cloud host cannot TCP 9100.",
