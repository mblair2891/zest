  ...
  topic({
    ...
    blocks: [
      ...
      ul(
        ...
        "Order tablet menu — My tables (full-service floor only; map + status, no side drawers). New table. To-go. Bar tab if the venue has a rail and the PIN allows it. Clock in/out. No sale / Print check / Pay only on stations bound to a Receipt printer. Unbound handhelds ring and fire. Closeout (server / manager, end of shift). Kitchen PIN on an order tablet: clock sheet — Clock and Done only.",
  ...
  topic({
    ...
    blocks: [
      ...
      steps(
        ...
        "Receipt printer: cash drawer kick yes/no. Two lists: Stations that may print (handhelds optional for paid receipts) and Stations that may kick drawer (terminals only). Station class Handheld vs Terminal. Card reader: mobile on each handheld, counter on each Lenovo-class terminal. Empty print list = no station prints from this printer. ODS never.",
        "Order printer: Destination is the production line this printer serves — Kitchen, Bar, Expo, Window, Prep, Salad, Pizza, Dessert, Other (default Kitchen). Add destination if the house has another line. Rename is allowed. Receipt printers have no destination field and never receive kitchen fire. Which stations may send: empty = every order and host station. Entity filter is Hearth vs Copper on a peer venue; Destination is which ticket printer on that side. Kitchen stays listed when Operating as is House.",
        "Save. On a paired Android station, Send / Print check / paid receipt write raw bytes to IP:9100 from the native shell. Guest Print check uses the Receipt printer (no destination; optional drawer kick). Send: each menu group’s destination + printer. Groups that share Kitchen and the same Star print as one slip. Drinks → Bar printer only (kitchen printer if no bar printer). QR, kiosk, online, and dashboard Test print write a venue print job — they cannot TCP 9100 from a guest phone or owner laptop. Dashboard Test print relays to an online station — “Printed via {station}” when that works. Unreachable from the cloud host is normal. Online paired stations and the house print agent subscribe to that queue and send 9100. Prefer a docked host or ODS so kitchen still prints when servers are in the dining room. If no worker is online, the ticket stays open and every station shows “N kitchen tickets waiting to print.” “Use a paired station or print agent” only when no station is online. Devices badge: LAN via station (green) or no station on LAN (amber) — not unreachable when a station is online and last print from that station succeeded. Never the OS print dialog.",
      ),
      ul(
        ...
        "Drawer kick: receipt printer only (Epson TM-T20 DK). Terminal station class + Stations that may kick drawer. Cash tender sends ESC/POS pulse (pin 2 default, or pin 5) on 9100 from this tablet or queued to any online station on the venue LAN. Print check does not kick unless Kick drawer on Print check is on (default off). If 9100 succeeds and the drawer stays shut, check the DK cable — the pulse is in the payload. Kitchen Star never kicks. Host and order tablets use the bound receipt printer; ODS does not need a receipt printer.",
