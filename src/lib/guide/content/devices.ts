...
export const DEVICE_TOPICS: GuideTopic[] = [
  ...
  topic({
    ...
    blocks: [
      ...
      steps(
        ...
        "BYOD phones are supported for running; leased Android stations are the always-on ODS/cashier path.",
      ),
      warn(
        "There is no separate “ODS appliance OS” in this product. A browser or the Android shell on the staff SSID is the device.",
      ),
  ...
  topic({
    ...
    summary: "Tablets, ODS, kiosks, and printers are location assets. Host assigns each to an entity and a function.",
    ...
    blocks: [
      why(
        "Hardware is not branded to a stall. The host location owns the registry. Any tablet can run Steam POS this week and Diamond ODS next week.",
      ),
      ul(
        "Type: tablet POS, order display, kiosk, printer, host stand, other.",
        "Assignment: operator (host or a guest entity) + function (floor POS, bar POS, kitchen ODS, bar ODS, expo, kiosk, host stand, cashier).",
        "On pair: the session picks up the assignment — menu scope, ticket routing, ODS station, permissions.",
      ...
      steps(
        ...
        "On The Laundry demo: Tablet A is Steam bar ODS, Tablet B is Diamond floor POS, Tablet C is host kiosk, 27\" is Diamond kitchen ODS.",
        "Reassign without replacing hardware. Ticket lines still carry the owning entity.",
      ),
      warn(
        "A Diamond ODS does not show Steam tickets unless the host grants Diamond view_tickets on Steam.",
      ),
