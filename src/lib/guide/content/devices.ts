...
export const DEVICE_TOPICS: GuideTopic[] = [
  topic({
    ...
    blocks: [
      ...
      ul(
        ...
        "Order tablet — full service opens Floor, Checks, Menu, Pay. The map is status color and the table number, no chairs. To-go, bar tab, and Close out are under Checks. The floor shows clocked in HH:MM. Print check follows the table’s section receipt printer. Handhelds ring and fire. Terminal cash kicks the receipt printer that printed that check. Kitchen PIN: Close out (End shift) and Done only. Counter is New ticket. Drive-through is the lane.",
        "Host tablet — Floor, Checks, Menu, Pay. Waitlist, To-go, and Close out are under Checks. No sale when this stand is bound to a Receipt printer. No ODS rail, no menu editor, no bar tab on this tablet.",
        "ODS — rail only plus Close out (End shift). No hamburger of POS tasks. A manager PIN on ODS is still the rail — not order entry.",
        "Counter / drive-through: New ticket or lane/window (no dining floor). Unpaired: type the Devices code and submit.",
        "Clock in is on the PIN pad and does not open order entry. Clock out is Close out on the short menu (or the ODS rail). The floor shows clocked in HH:MM. Live venues never show Operating as on the staff menu. Isolated demos: one small control in a manager More overflow.",
      ...
      steps(
        ...
        "To retire a named station: Delete (one confirm). It stays gone after deploy. Factory reset / Reseed demo: “This restores demo stations and printers.” To keep the name and swap hardware: Replace or Unpair. To leave it in the list: Deactivate — the tablet returns to the pair-code field; Activate again shows a new code.",
      ),
      warn(
        "Do not send kitchen staff to /login. A kitchen PIN on an order tablet is the clock sheet — Close out and Done — not the server UI. Enter is not a punch. Clock in is the pad key. Kitchen Close out is End shift (punch out only).",
      ),
  ...
  topic({
    ...
    blocks: [
      ...
      tip(
        "Host stand: Floor, Checks, Menu, and Pay are the bar. Waitlist and To-go are under Checks. Same on a handheld as on the counter.",
      ),
  ...
  topic({
    ...
    blocks: [
      ...
      steps(
        ...
        "After menu or floor layout edits: Publish changes. Stations prompt on the next heartbeat. 86 / un-86 is not a publish — every station sees it immediately.",
        ...
        "Website deploys (new JS on app.summex.app): the station modal “A system update is ready.” Update now reloads the WebView (appBuild) or refetches menus, printers, and taxes (configVersion). Remind me later hides it for 10 minutes; after three snoozes a small persistent bar stays until they update. Staff may ignore optional updates all shift. Venue Settings → Updates: force-update window (default 04:00 venue IANA, optional second window). During that hour, if the station is behind: modal “Update required” with only Update now (no snooze). Idle stations apply after 60 seconds if nobody taps. If the tablet was off, offline, or asleep through the window and is still behind: next heartbeat shows “An update was waiting while this station was offline.” Update now is primary. Remind me later only outside the window when Catch-up is mandatory is off (default on — no snooze after a missed window). Same change-list bullets as the live prompt. Show change list (default on) is station-facing only (print, pay, floor, PIN). Never reload without a tap outside the force window. PIN pad and idle floor may show the modal immediately. Update now during send, pay, or Print check toasts “Finish this check first.” Do not force-stop, unpair, or install a new APK for a website deploy.",
        "APK updates (new Summex Station binary): from a signed-in POS session, Exit kiosk ends the session and returns to the PIN pad. From the PIN pad, Exit kiosk opens one dialog, Manager / station PIN, with Cancel and one Exit kiosk button. A valid manager, owner, or Devices service PIN leaves lock-task for Android home. A staff PIN stays pinned and toasts. Long-press reload (2 seconds) uses the same PIN rule.",
      ),
      ul(
        ...
        "Exit kiosk on the station shell. A signed-in session ends on the PIN pad. On the PIN pad, one dialog asks for the manager or station PIN. Cancel or Exit kiosk. That PIN stops lock-task. If Android keeps the pin (the tablet is not device-owner), the station says to unpin from the recents screen and still leaves the Summex shell for Android home. Staff PINs cannot exit.",
        "Devices → Station service PIN: optional 4-digit PIN with the same reload/exit rights as manager.",
      ),
      warn(
        "Do not put guest QR / pay on this APK. Table tents and ticket codes open in the guest’s browser. Do not force-stop, unpair, or install a new APK for a website deploy — tap Update now on the station.",
      ),
  ...
  topic({
    ...
    blocks: [
      ...
      p(
        "Devices → Add device. Type is Receipt printer or Order printer — same list as tablets. Kitchen, bar, salad, pizza, dessert, and label are destinations on an order printer, not extra types. Form fields, not JSON: name, make/model, Wi-Fi or Ethernet, static IP (required for live print; pending IP is allowed on a slot), port (default 9100).",
      ),
      steps(
        ...
        "Receipt printer: assign to floor sections (and optionally Bar tabs / no section). Venue default is to-go / will-call. Print pay QR (default on for TM-T20) — Epson guest check / paid receipt include a native thermal QR for that check when venue QR pay-or-reorder is on. Cash drawer kick yes/no. Stations that may kick drawer (terminals only). A station may list a fallback receipt printer if the section has none — that is not routing. ODS never.",
        "Order printer: Destination is the production line this printer serves — Kitchen, Bar, Expo, Window, Prep, Salad, Pizza, Dessert, Other (default Kitchen). Add destination if the house has another line. Rename is allowed. Receipt printers have no destination field and never receive kitchen fire. Food printers are assigned on Menu groups, not by table section. Bar printers are assigned to floor sections (bar-rail / well is a section). If only one bar printer exists, every section maps to it. Entity filter is Hearth vs Copper on a peer venue; Destination is which ticket printer on that side. Kitchen stays listed when Operating as is House.",
        "Save. On a paired Android station, Send / Print check / paid receipt write raw bytes to IP:9100 from the native shell. Order / host tablets fire; the venue map picks the IP. Guest Print check uses the receipt printer for that table’s section (venue default if no table). Send: each menu group’s destination + printer. Groups that share Kitchen and the same Star print as one slip. Drinks → the bar printer for that table’s section (kitchen printer if no bar printer). QR, kiosk, online, and dashboard Test print write a venue print job — they cannot TCP 9100 from a guest phone or owner laptop. Dashboard Test print relays to an online station — “Printed via {station}” when that works. Unreachable from the cloud host is normal. Online paired stations and the house print agent subscribe to that queue and send 9100. Prefer a docked host or ODS so kitchen still prints when servers are in the dining room. If no worker is online, the ticket stays open and every station shows “N kitchen tickets waiting to print.” “Use a paired station or print agent” only when no station is online. Printers are not clients. Devices badge: LAN via {station name} (green) + that station’s heartbeat when any paired order / host / ODS tablet has a heartbeat in the last 90 seconds — order tablets count (they send 9100). no station on LAN (amber) only when no station heartbeat is fresh. Do not leave printers amber while a station row is green. Station online is the same 90-second heartbeat — last seen yesterday is offline. Never the OS print dialog.",
      ),
      ul(
        "Production: Ethernet to the staff AP. Do not join the printer to guest Wi‑Fi or run it as its own hotspot.",
        "Star SP742 at 192.168.0.105:9100: Order printer, destination Kitchen (or Bar / Expo / Window / Prep / Salad / Pizza / Dessert), make/model Star SP700 / SP742. That preset is text-only 9-pin impact (Star Line 7x9, CP437, single pass, no GS graphics, no TM-T20 payload). Kitchen ticket: venue, destination, server, venue-timezone time, entity, qty + item. Never a pay QR. Header may be double-height only. Staff Send from a paired order tablet writes TCP 9100. Guest Print check is the receipt printer (example 192.168.0.112), not this Star — that Epson prints a native pay QR when venue QR pay-or-reorder is on and Print pay QR is checked (default on for TM-T20). QR / kiosk / online fire kitchen bytes via the venue queue. Entity filter is Hearth vs Copper; Destination is the line on that side.",
        "Drawer kick: receipt printer only (Epson TM-T20 DK). Terminal station class + Stations that may kick drawer. Cash tender kicks the receipt printer that printed that check (the section Epson). Pulse pin 2 default, or pin 5. Print check does not kick unless Kick drawer on Print check is on (default off). If 9100 succeeds and the drawer stays shut, check the DK cable — the pulse is in the payload. Kitchen Star never kicks. ODS does not need a receipt printer.",
        ...
        "Isolated demo (Summit Hall) seeds a receipt slot plus kitchen and bar order printers once (IP can stay pending). Delete Kitchen printer and it stays gone on the next deploy. Live subscriber venues do not seed printers — Add device when the house buys them. Factory reset / Reseed demo: “This restores demo stations and printers.”",
        "Bring-your-own Android tablets run POS/ODS via Summex Station. Printers and drawers stay BYO.",
      ),
      warn(
        "There is no Kitchen printer, Bar printer, or Label printer type. Those are destinations on an Order printer. Destination is the line it serves — not a menu group. Star SP700/SP742 tickets are impact text (7x9, no GS bitmaps, no leading !, not doubled thermal). Kitchen tickets never use window.print. Guest checks print on the Receipt printer. Receipt printers never get kitchen fire. QR, kiosk, and online do not print from the guest browser — they queue. Dock a host or ODS (or run the print agent on the house hub) so the queue drains. Do not put order printers on the printer’s own Wi‑Fi.",
      ),
      related("wifi-offline", "kds", "device-roles", "cash-handling", "station-switcher", "sections", "kitchen-bar-routing"),
    ],
  }),
  topic({
