...
export const CASH_GIFT_TOPICS: GuideTopic[] = [
  ...
  topic({
    ...
    blocks: [
      ...
      steps(
        ...
        "Transfer cash when you are short on small bills and another open till has them. You are the requester (you receive). Pick the giving till, an amount, optional mix, optional note. That till Accepts or Declines — no manager required. Cash moves at accept, not at request. Both tills print a TILL TRANSFER slip. You may cancel while pending. After accept, only a manager can reverse.",
        "Count your till is always blind: enter what is in the drawer (bills/coin or a single total). Expected, cash sales, and over/short stay hidden until submit. After submit the receipt printer prints a turn-in slip for the drop bag. Mid-shift drops already left the till — do not count them again. Resolve any pending till transfer first.",
  ...
  topic({
    ...
    blocks: [
      ...
      steps(
        "On the order station tap End shift (not Labor, not PIN login).",
