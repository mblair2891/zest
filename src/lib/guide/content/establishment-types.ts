...
export const ESTABLISHMENT_TYPE_TOPICS: GuideTopic[] = [
  ...
  topic({
    ...
    blocks: [
      ...
      p(
        "Two models. Host + tenants: a host subscriber owns the floor and may sell. Host owner/manager password login has full access to every tenant’s ops; tenant logins stay on their own slice. Shared venue: a named building only — no host merchant, menu, gift product, or host role. Venue admin is not a landlord brand. Lines keep their operator. Kitchen and bar rails stay apart. The guest tenders once; Finix splits; receipts group by vendor.",
      ),
      ul(
