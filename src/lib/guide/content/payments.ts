...
export const PAYMENT_TOPICS: GuideTopic[] = [
  ...
  topic({
    ...
    blocks: [
      ...
      p(
        "Print check (before tender) and the paid receipt are different. Print check appears on the order pad and on Pay when a receipt printer is assigned to that table’s floor section (Epson TM-T20 at the venue IP:9100 — house example 192.168.0.112). To-go / will-call use the venue default receipt printer. It is the guest’s itemized bill — not the kitchen Star. Each item has cash / card beside it. Named tax lines only when rates exist — zero rates means no tax line and no extra cents. CASH TOTAL is the sum of line cash prices; CARD TOTAL is the sum of line card prices. Cash discount is not applied again on the grand total. Footer: “Not a receipt - pay server.” If that Epson has Print pay QR on (default on for TM-T20), the slip includes a native thermal QR of that check’s public URL — reorder + pay, no staff PIN. Checkbox on with no QR is a bug. Star kitchen tickets never get a pay QR. Clock is the venue IANA timezone with AM/PM (never UTC, never the tablet). No guest split on that ticket — staff already split checks. If no receipt printer is assigned: “Add a receipt printer in Devices.” Cash, Card, and Gift still show. After the guest pays: Email, SMS, Printed receipt (same section Epson), or None. A handheld queues the paid receipt to that section printer or Open on terminal so a register reprints. Quantum Payments is the tender line — guests never see Finix.",
      ),
      ul(
