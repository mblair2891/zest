...
export const PAYMENT_TOPICS: GuideTopic[] = [
  ...
  topic({
    ...
    blocks: [
      ...
      p(
        "Print check (before tender) and the paid receipt are different. Print check appears on the order pad and on Pay when a receipt printer is assigned to that table’s floor section (Epson TM-T20 at the venue IP:9100 — house example 192.168.0.112). To-go / will-call use the venue default receipt printer. It is the guest’s itemized bill — not the kitchen Star. Lines group under the selling entity (Hearth / Copper / …) with qty, cash price and card price when cash discount is on, subtotal per entity, each named tax line (Sales, Restaurant, …), grand cash total, grand card total, and “Not a receipt — pay server.” Zero tax rates: no tax line. When venue QR pay-or-reorder is on and that Epson has Print pay QR (default on for TM-T20), the guest check includes a native thermal pay QR for that check — reorder + pay, no staff PIN. Star kitchen tickets never get a pay QR. Clock on the ticket is the venue IANA timezone. No guest split on that ticket — staff already split checks. If no receipt printer is assigned: “Add a receipt printer in Devices.” Cash, Card, and Gift still show. After the guest pays: Email, SMS, Printed receipt (same section Epson), or None. A handheld queues the paid receipt to that section printer or Open on terminal so a register reprints. Quantum Payments is the tender line — guests never see Finix.",
      ),
      ul(
