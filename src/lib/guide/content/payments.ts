...
export const PAYMENT_TOPICS: GuideTopic[] = [
  ...
  topic({
    ...
    blocks: [
      ...
      p(
        "Open White paper from the marketing header or footer (no login). Print from the browser for a PDF. Revision · 22 Sep 2026 matches Guide v2026.10.144. A full-service demo publishes its dining room and bar rail, and the live floor draws that snapshot. Each selling entity uploads its menu as a draft, then publishes that entity’s items to the order pad. The live floor is a wood map. Exit kiosk leaves lock-task. The guest-check header is the building name in text. Location and entity logos are on the QR page and tablets. It is written for prospective subscribers: one guest check, floor, multi-entity, Android staff stations, venue payment-method toggles, 5% cash-discount processing story, plans from Get a price. Gift cards are not sold online. QR is on-premise. No CRM, pipeline, factory reset, or how to log in. Internal operations notes stay off the public site.",
      ),
      steps(
