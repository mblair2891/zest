...
export const FLOOR_TOPICS: GuideTopic[] = [
  topic({
    ...
    blocks: [
      ...
      steps(
        ...
        "Tap a table. If any OPEN checks exist for that table, table view opens — not Empty / Seat. Header: table #, section, covers, server. Every open check is listed (#, server, cash total, card total, item count) with a large Print check on the row. Print all open is secondary. Print check for a Dining table goes to receipt printers assigned to Dining — this station sends 9100 to that Epson immediately. Job is a guest check / receipt, never kitchen / Star / “kitchen tickets waiting.” The Epson slip includes a pay QR for that check when venue QR pay-or-reorder is on and Print pay QR is checked, named tax lines, and the venue timezone. Toast: “Sending to Dining receipt printer” or “No receipt printer for this section — add one in Devices.” Never silent. Fallback: venue default receipt printer, then any receipt printer; last resort is that error. Entity filter on the receipt printer does not block a house check. Tap the check to add, void, send, or pay. Actions: New check on this table, Transfer / hold, Table QR. Staff split stays on the check, not guest QR.",
  ...
  topic({
    ...
    summary: "Short host menu: Floor / seat, Waitlist, To-go, Clock. Map + status only.",
    roles: ["owner_manager", "server"],
    ...
    blocks: [
      ...
      steps(
        "PIN in on the host tablet. Pick Floor / seat. Add a walk-in from Waitlist with party size and quoted wait.",
