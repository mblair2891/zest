  ...
  topic({
    ...
    blocks: [
      ...
      ul(
        ...
        "Floor is a white workspace, full bleed. An empty table is a dark ring with a clear center and the number inside. Any other status fills that shape with the status color and a high-contrast number. Round tables are circles. Other tables are rounded squares. A few seat nubs sit on round and square tables. The bar is a gray BAR slab. Stools are B1…Bn tiles on the guest edge. Walls, doors, and windows stay dark on the white. Top shows the staff name and local time, plus a tight icon row for messages, print, and switch user. Bottom left is Dine in | To go. The other corner is Tickets search. Tap a table with open checks — one tap — for Add, Send, Print check, Pay. Empty + Seat only when vacant. If the published floor has tables, they are drawn. If that snapshot is empty, the last saved draft is drawn and the banner says Publish floor to lock layout. A failed picture or a demo seed does not hide the tables.",
  ...
  topic({
    ...
    blocks: [
      ...
      steps(
        ...
        "Return to Floor on an order or host tablet. The editor and the live floor are a white workspace. Walls, doors, windows, the host stand, and the bar slab stay dark in the same place as the editor. A house name can sit small on the line. Empty dining tables are hollow rings with the number inside and a few seat nubs. Any other status fills the circle or rounded square and keeps those nubs. Square no seats has none. The bar slab and B tiles match the editor. Publish writes the full layout, so the editor and the live map show the same pieces.",
      ),
      tip(
