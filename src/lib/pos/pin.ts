import type { Employee, EmployeeRole } from "./types";
import { HOST_SCOPE } from "@/lib/access/entity-grants";
import { isDemoStaffPin } from "@/lib/demo/pin";

export const FLOOR_ROLES: EmployeeRole[] = [
  "server",
  "host",
  "bartender",
  "kitchen",
  "busser",
  "cashier",
];

export const BACK_OFFICE_ROLES: EmployeeRole[] = [
  "owner",
  "manager",
  "accountant",
  "vendor_operator",
];

export function isFloorRole(role: EmployeeRole | null | undefined): boolean {
  return !!role && FLOOR_ROLES.includes(role);
}

export function isBackOfficeRole(role: EmployeeRole | null | undefined): boolean {
  return !!role && BACK_OFFICE_ROLES.includes(role);
}

export function isFourDigitPin(pin: string): boolean {
  return /^\d{4}$/.test(pin);
}

/** Compact SHA-256 (hex). PINs are hashed at rest — never store plaintext. */
export function sha256Hex(message: string): string {
  const bytes = utf8Bytes(message);
  const H = new Uint32Array([
    0x6a09e667, 0xbb67ae85, 0x3c6ef372, 0xa54ff53a, 0x510e527f, 0x9b05688c, 0x1f83d9ab,
    0x5be0cd19,
  ]);
  const K = SHA_K;
  const l = bytes.length;
  const withOne = l + 1;
  const paddedLen = Math.ceil((withOne + 8) / 64) * 64;
  const buf = new Uint8Array(paddedLen);
  buf.set(bytes);
  buf[l] = 0x80;
  const view = new DataView(buf.buffer);
  view.setUint32(paddedLen - 4, l * 8, false);
  const w = new Uint32Array(64);
  for (let i = 0; i < paddedLen; i += 64) {
    for (let t = 0; t < 16; t++) w[t] = view.getUint32(i + t * 4, false);
    for (let t = 16; t < 64; t++) {
      const s0 = rotr(w[t - 15]!, 7) ^ rotr(w[t - 15]!, 18) ^ (w[t - 15]! >>> 3);
      const s1 = rotr(w[t - 2]!, 17) ^ rotr(w[t - 2]!, 19) ^ (w[t - 2]! >>> 10);
      w[t] = (w[t - 16]! + s0 + w[t - 7]! + s1) >>> 0;
    }
    let [a, b, c, d, e, f, g, h] = H;
    for (let t = 0; t < 64; t++) {
      const S1 = rotr(e, 6) ^ rotr(e, 11) ^ rotr(e, 25);
      const ch = (e & f) ^ (~e & g);
      const temp1 = (h + S1 + ch + K[t]! + w[t]!) >>> 0;
      const S0 = rotr(a, 2) ^ rotr(a, 13) ^ rotr(a, 22);
      const maj = (a & b) ^ (a & c) ^ (b & c);
      const temp2 = (S0 + maj) >>> 0;
      h = g;
      g = f;
      f = e;
      e = (d + temp1) >>> 0;
      d = c;
      c = b;
      b = a;
      a = (temp1 + temp2) >>> 0;
    }
    H[0] = (H[0]! + a) >>> 0;
    H[1] = (H[1]! + b) >>> 0;
    H[2] = (H[2]! + c) >>> 0;
    H[3] = (H[3]! + d) >>> 0;
    H[4] = (H[4]! + e) >>> 0;
    H[5] = (H[5]! + f) >>> 0;
    H[6] = (H[6]! + g) >>> 0;
    H[7] = (H[7]! + h) >>> 0;
  }
  return Array.from(H)
    .map((n) => n.toString(16).padStart(8, "0"))
    .join("");
}

export function hashPin(pin: string, locationId: string): string {
  const p = pin.replace(/\D/g, "").slice(0, 4);
  return sha256Hex(`summex-pin:v1:${locationId || "loc"}:${p}`);
}

export function pinMatches(
  emp: Pick<Employee, "pin" | "pinHash" | "active">,
  pin: string,
  locationId: string,
): boolean {
  if (!emp.active) return false;
  if (!isFourDigitPin(pin)) return false;
  const hashed = hashPin(pin, locationId);
  if (emp.pinHash && emp.pinHash === hashed) return true;
  if (emp.pin && emp.pin === pin) return true;
  return false;
}

export function findStaffByPin(
  employees: Employee[],
  pin: string,
  locationId: string,
  deviceOperatorId?: string | null,
): Employee | undefined {
  let matches = employees.filter((e) => pinMatches(e, pin, locationId));
  if (!matches.length && isDemoStaffPin(pin)) {
    matches = employees.filter((e) => e.active);
  }
  const deviceOp = deviceOperatorId && deviceOperatorId !== HOST_SCOPE ? deviceOperatorId : null;
  if (deviceOp) {
    return (
      matches.find((e) => e.operatorId === deviceOp) ??
      matches.find((e) => e.role === "owner" || e.role === "manager")
    );
  }
  return matches[0];
}

export function withHashedPin<T extends { pin?: string; pinHash?: string }>(
  emp: T,
  pin: string,
  locationId: string,
): T {
  if (!isFourDigitPin(pin)) return emp;
  return { ...emp, pinHash: hashPin(pin, locationId), pin: "" };
}

function rotr(x: number, n: number): number {
  return (x >>> n) | (x << (32 - n));
}

function utf8Bytes(s: string): Uint8Array {
  if (typeof TextEncoder !== "undefined") return new TextEncoder().encode(s);
  const out: number[] = [];
  for (let i = 0; i < s.length; i++) {
    const c = s.charCodeAt(i);
    if (c < 128) out.push(c);
    else if (c < 2048) out.push(192 | (c >> 6), 128 | (c & 63));
    else out.push(224 | (c >> 12), 128 | ((c >> 6) & 63), 128 | (c & 63));
  }
  return Uint8Array.from(out);
}

const SHA_K = new Uint32Array([
  0x428a2f98, 0x71374491, 0xb5c0fbcf, 0xe9b5dba5, 0x3956c25b, 0x59f111f1, 0x923f82a4, 0xab1c5ed5,
  0xd807aa98, 0x12835b01, 0x243185be, 0x550c7dc3, 0x72be5d74, 0x80deb1fe, 0x9bdc06a7, 0xc19bf174,
  0xe49b69c1, 0xefbe4786, 0x0fc19dc6, 0x240ca1cc, 0x2de92c6f, 0x4a7484aa, 0x5cb0a9dc, 0x76f988da,
  0x983e5152, 0xa831c66d, 0xb00327c8, 0xbf597fc7, 0xc6e00bf3, 0xd5a79147, 0x06ca6351, 0x14292967,
  0x27b70a85, 0x2e1b2138, 0x4d2c6dfc, 0x53380d13, 0x650a7354, 0x766a0abb, 0x81c2c92e, 0x92722c85,
  0xa2bfe8a1, 0xa81a664b, 0xc24b8b70, 0xc76c51a3, 0xd192e819, 0xd6990624, 0xf40e3585, 0x106aa070,
  0x19a4c116, 0x1e376c08, 0x2748774c, 0x34b0bcb5, 0x391c0cb3, 0x4ed8aa4a, 0x5b9cca4f, 0x682e6ff3,
  0x748f82ee, 0x78a5636f, 0x84c87814, 0x8cc70208, 0x90befffa, 0xa4506ceb, 0xbef9a3f7, 0xc67178f2,
]);
