# Zest Hardware Policy (v1.0)

**BYOD-first software. Certified payments & printers. Optional buy · finance · subscribe.**

Proprietors: Michael Blair & Andy Baida · Product: Zest Hospitality OS

---

## One-page summary

| Path | Customer pays | Owns hardware? | Best for |
|---|---|---|---|
| **BYOD** | Devices they already have | Yes | Default; hall vendors; truck pods |
| **Buy kit** | One invoice | Yes day one | Stable sites with cash |
| **Partner finance** | 24–36 mo payments → ~$1 buyout | Yes after term | Want kit without CapEx |
| **Device subscription** | $/device/mo + swap SLA | Zest (fleet) | Always-on KDS / managed counters |

### Non‑negotiables
1. Software runs in browser/PWA on **customer tablets/phones** by default.  
2. **Live card-present** = **Stripe Terminal** certified readers only.  
3. Printers = **Star Micronics** or **Epson** models on the certified list.  
4. Hardware contract **≠** software SaaS **≠** processing rate (quote separately).  
5. No silent 48‑month ISO traps; finance only via **named partner**, terms disclosed.  
6. Halls: **vendors BYOD**; host may equip shared KDS/network/settlement station.  
7. Pods: **BYOD-first**; pad power/network is site infrastructure.

### Finance guardrails
- Prefer **24–36 months**; show **total of payments vs buy**.  
- ETF / personal guarantee called out on partner paper.  
- v1: **Zest does not hold leases** — partner lessor funds.

### Illustrative kits (USD list; street varies)

| Kit | Buy ≈ | Finance ≈ /mo (36) | Subscribe ≈ /mo |
|---|---|---|---|
| Counter Ready | $1,198 | $42 | $90 |
| Handheld Service | $558 | $22 | $47 |
| KDS Station | $539 | $18 | $46 |
| Hall Host Pack | $1,458 | $55 | $109 |

---

## Device sources (official / primary)

| Device | Source |
|---|---|
| Stripe Terminal catalog | https://stripe.com/terminal/devices |
| Stripe Reader M2 | https://stripe.com/terminal/m2 |
| Stripe Reader S700 | https://stripe.com/terminal/s700 |
| Apple iPad | https://www.apple.com/ipad/ |
| Apple iPad mini | https://www.apple.com/ipad-mini/ |
| Samsung tablets | https://www.samsung.com/us/tablets/ |
| Star thermal printers | https://starmicronics.com/thermal-pos-receipt-printers/ |
| Epson POS printers | https://epson.com/For-Work/Printers/POS/c/w330 |
| APG cash drawers | https://www.apgcashdrawer.com/ |
| Heckler mounts | https://www.hecklerdesign.com/ |
| Ubiquiti networking | https://www.ui.com/ |
| Cisco Meraki | https://meraki.cisco.com/ |

Full interactive catalog: **Platform → Devices** in Zest SaaS (`/platform`).
