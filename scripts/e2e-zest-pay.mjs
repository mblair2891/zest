...
await page.screenshot({ path: "/workspace/screenshots/zest-payments.png" });

const body = await page.locator("body").innerText();
const hasZest = /Zest Payments/i.test(body);
const hasStripe = /\bStripe Payments\b|\bSquare Payments\b/.test(body);
const builtIn = /not a partner you pick|built into Zest/i.test(body);
...
await page.screenshot({ path: "/workspace/screenshots/zest-gifts.png" });
const guestsText = await page.locator("body").innerText();
const giftLedger = /Zest gift ledger|First-party/i.test(guestsText);
...
await browser.close();
process.exit(hasZest && !hasStripe && giftLedger && errors.length === 0 ? 0 : 1);
