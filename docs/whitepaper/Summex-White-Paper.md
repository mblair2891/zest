...
- **Drink revenue share** — a peer venue or a hosted hall can send a percent of drink net (after comps and voids) from the bar entity to the food entity, for the whole venue or for chosen sections and tables. A dining rule does not include a bar tab with no dining table. It is a second journal. The guest check, tax, and the Quantum Payments card split stay with whoever sold the item. The period can show a Finix split instruction or a book entry for the amount they settle offline.
