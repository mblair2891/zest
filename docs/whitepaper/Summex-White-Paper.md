...
Printers are two types on Devices: **Receipt printer** (guest check, pay, gift, drawer kick, pay QR; no destination) and **Order printer** (fire tickets to a named line — Kitchen, Bar, Expo, Window, Prep, Other, or a house-named line — plus entity or all, and which stations may send). Destination defaults to Kitchen. Pick the make/model the house bought (Star SP700/SP742, Epson TM, Citizen, Bixolon, generic ESC/POS); that preset sets emulation, paper, cutter, and port 9100. Paired Android writes those bytes from the native shell (WebView cannot TCP 9100). Laptop Chrome uses a print agent. Test print never uses the computer’s print dialog. Ethernet on the house AP LAN, not the printer’s own Wi‑Fi. Receipts thermal; kitchen impact.
