...
**Revision · 17 Sep 2026** — Printer LAN via station.
Guide v2026.10.108.
...
14. Printers: **two types only** — Receipt printer (guest check, pay, gift, drawer kick, pay QR; no destination) and Order printer. Destination is the production line: Kitchen, Bar, Expo, Window (plus Prep / Other / house-named). Route by item entity + destination. Default Kitchen. Paired Android on house Wi-Fi prints via native TCP 9100 (WebView is HTTPS and cannot open 9100). Devices row: **LAN via station** (green) when a paired station is online — never “unreachable” after a successful fire/test from that station. **no station on LAN** (amber) when none are online. Dashboard Test print queues to an online station. “Use a paired station or print agent” only if zero stations are online. **Star SP700 / SP742 / SP712** is 9-pin impact (not TM-T20). Epson TM-T20 / T88 / m30 stay thermal.
15. Offline / hybrid
