...
14. Printers: **two types only** — Receipt printer (guest check, pay, gift, drawer kick, pay QR; no destination) and Order printer. Destination is the production line: Kitchen, Bar, Expo, Window, Prep, Other, or a house-named line (Add destination). Default Kitchen. Entity filter is Hearth vs Copper on a peer venue — not the line. Do not hide Kitchen when Operating as is House. Make/model preset (Star SP700/SP742, Epson TM, generic ESC/POS) sets emulation, paper width, cutter, port 9100. Test print = raw bytes to IP:9100 — never the OS print dialog.
15. Offline / hybrid
