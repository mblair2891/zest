...
**Revision · 16 Sep 2026** — Station raw 9100 print.
Guide v2026.10.106.
...
14. Printers: **two types only** — Receipt printer (guest check, pay, gift, drawer kick, pay QR; no destination) and Order printer. Destination is the production line: Kitchen, Bar, Expo, Window (plus Prep / Other / house-named). Route by item entity + destination. Default Kitchen. Paired Android on house Wi-Fi prints via native TCP 9100 (WebView is HTTPS and cannot open 9100). Laptop Chrome uses the LAN print agent, else “Use a paired station or print agent.” Test print never uses the OS print dialog. Make/model (Star SP700/SP742, Epson TM, generic ESC/POS) sets emulation, paper, cutter, port 9100.
15. Offline / hybrid
